...
class Task {
    constructor(task_id, task_counter, start_time, finish_time) {
...
class SinglyLinkedList {
    ...
    add(task_id, task_counter, start_time, finish_time) {
        var node = new Task(task_id, task_counter, start_time, finish_time);
    ...
    insertAt(index, task_id, task_counter, start_time, finish_time) {
    ...
    insertAfter(task_node, task_id, task_counter, start_time, finish_time) {
        var new_node = new Task(task_id, task_counter, start_time, finish_time);
    ...
    removeFrom(index) {
        if (index < 0 || index > this.size)
            ...
            this.size --;
            return [curr.task_id, curr.task_counter] ;
        }
    }

    removeTask(task_id, task_counter) {
        var current = this.head;
        var prev = null;

        while (current != null) {
            if (current.task_id === task_id && current.task_counter === task_counter) {
                if (prev == null) {
                ...
                this.size --;
                return [current.task_id, current.task_counter];
            }
            prev = current;
            current = current.next;
        }
    ...
    printList() {
        ...
        while (curr) {
            str += 'task_id: ' + curr.task_id + ' task_counter:' + curr.task_counter + ' start_time:' + curr.start_time + ' finish_time:' + curr.finish_time + " \n-> ";
            curr = curr.next;
        }
    ...
    toArray() {
        ...
        while (current != null) {
            //let task = [current.task_id, current.task_counter, current.start_time, current.finish_time];
            let task = [current.task_id, current.start_time, current.finish_time];
            arr.push(task);
            current = current.next;
        }
        return arr;
